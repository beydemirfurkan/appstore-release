...
export async function run({ client, discovery, options = {} }) {
  ...
  if (current?.id === build.id) return { status: Status.OK, message: `build ${build.attributes.version} already attached` };

  await client.patch(`/v1/appStoreVersions/${version.id}/relationships/build`, { data: { type: "builds", id: build.id } });
