...
export async function run({ client, discovery, log }) {
  ...
  log.section("App");
  log.info(
    `${app.attributes.name} • ${app.attributes.bundleId} • SKU ${app.attributes.sku} • ${app.attributes.primaryLocale}`,
  );
  ...
  for (const b of builds.data)
    log.info(
      `build ${b.attributes.version} • ${b.attributes.processingState}${b.attributes.expired ? " • EXPIRED" : ""} • ${b.attributes.uploadedDate?.slice(0, 10)}`,
    );
  ...
  log.info(
    `primary:${info?.relationships?.primaryCategory?.data?.id || "none"} • secondary:${info?.relationships?.secondaryCategory?.data?.id || "-"}`,
  );
  ...
  for (const s of all)
    log.info(
      `${s.attributes.name} • ${s.attributes.productId} • ${s.attributes.subscriptionPeriod} • ${s.attributes.state}`,
    );
  ...
  for (const c of certs.data)
    log.info(
      `${c.attributes.certificateType} • ${c.attributes.displayName || c.attributes.name} • exp:${c.attributes.expirationDate?.slice(0, 10)}`,
    );
  const profiles = await client.get(
    `/v1/profiles?limit=30&fields[profiles]=name,profileType,profileState,expirationDate`,
  );
  ...
  for (const p of mine.length ? mine : profiles.data.slice(0, 6))
    log.info(
      `${p.attributes.name} • ${p.attributes.profileType} • ${p.attributes.profileState} • exp:${p.attributes.expirationDate?.slice(0, 10)}`,
    );
